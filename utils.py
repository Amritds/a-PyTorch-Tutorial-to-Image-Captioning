...
def compute_cider(references, hypothesis):
    ...
    requests.get('http://0.0.0.0:5000/cider', params = params)
...
def cider_reward(imgs, hypothesis, save_imgs, ground_truth):
    ...
    ground_truth_sentence = [' '.join([rev_word_map[ind] for ind in sent if ind not in {word_map['<start>'], word_map['<end>'], word_map['<pad>']}]) for sent in ground_truth]
